        ...
        <div className="flex flex-col md:flex-row gap-8 w-full">
          ...
          <section className="flex-1 min-w-[300px]">
            ...
            <div className="flex gap-2 mt-4 justify-center md:justify-start">
              <button onClick={() => setUserPage((p) => Math.max(1, p - 1))} disabled={userPage === 1} className="px-3 py-1 bg-gray-700 rounded disabled:opacity-50">Prev</button>
              <span>Página {userPage} de {totalUserPages}</span>
              <button onClick={() => setUserPage((p) => Math.min(totalUserPages, p + 1))} disabled={userPage === totalUserPages} className="px-3 py-1 bg-gray-700 rounded disabled:opacity-50">Next</button>
            </div>
          </section>
          ...
          <section className="flex-1 min-w-[300px]">
            ...
            <div className="flex flex-col gap-4">
              {(Array.isArray(budgets) ? budgets : []).slice(0, 6).map((b) => (
                <div key={b.id} className="bg-gray-800 rounded-lg p-4 flex flex-col shadow-md">
                  <span className="font-bold">{b.codigo}</span>
                  <span className="text-lg text-gray-300"> {b.person.nombre}</span>
